...
func CreateOrgRepository(provider gitprovider.Client,orgRepoRef gitprovider.OrgRepositoryRef,repoInfo gitprovider.RepositoryInfo) error {

	ctx := context.Background()
	defer ctx.Done()

	_, err := provider.OrgRepositories().Create(ctx, orgRepoRef,repoInfo)
	if err != nil {
...
func CreateUserRepository(provider gitprovider.Client,userRepoRef gitprovider.UserRepositoryRef,repoInfo gitprovider.RepositoryInfo) error{

	ctx := context.Background()
	defer ctx.Done()

	_, err := provider.UserRepositories().Create(ctx, userRepoRef, repoInfo)
	if err != nil {
		return fmt.Errorf("error creating repo %s",err)
	}
