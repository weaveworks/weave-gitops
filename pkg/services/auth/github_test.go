...
var _ = Describe("GitHub error parsing", func() {
	It("parses from a response", func() {
		err := auth.ParseGitHubError([]byte(`{"error":"device_flow_disabled","error_description":"Device Flow must be explicitly enabled for this App","error_uri":"https://docs.github.com"}`), http.StatusBadRequest)
...
var _ = Describe("ValidateToken", func() {
	It("returns unauthenticated on an invalid token", func() {
		...
		gh := auth.NewGithubAuthClient(&http.Client{Transport: rt})
