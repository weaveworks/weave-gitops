...
func CleanupBackgroundProxy(proxyName string, log log.Logger) error {
	...
	if containerExists(proxyName) {
		...
		log.Actionf("Stopping background proxy...")
		_, _ = cmd.Output()
	}
	return nil
}
...
func createProxyContainer(vClusterName, vClusterNamespace string, rawConfig, vRawConfig *clientcmdapi.Config, service *corev1.Service, localPort int, timeout time.Duration, backendHost, network string, log log.Logger) (string, error) {
	...
	log.Actionf("Starting proxy container...")
	out, err := cmd.Output()
	if err != nil {
...
func CreateBackgroundProxyContainer(vClusterName, vClusterNamespace string, rawConfig, vRawConfig *clientcmdapi.Config, localPort int, log log.Logger) (string, error) {
	...
	log.Actionf("Starting background proxy container...")
	out, err := cmd.Output()
	if err != nil {
...
func getServerFromExistingProxyContainer(vClusterName, vClusterNamespace string, rawConfig, vRawConfig *clientcmdapi.Config, service *corev1.Service, log log.Logger) (string, error) {
	...
	if err == nil {
		...
		log.Failuref("Error running docker inspect with go template: %v", err)
	}
