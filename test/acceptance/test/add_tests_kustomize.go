...
var kustomizeClusterName string
...
var _ = Describe("Weave GitOps App Add Tests", func() {
	...
	var _ = BeforeEach(func() {
		By("Given I have a brand new cluster", func() {
			...
			kustomizeClusterName = getClusterName()
		})
	...
	It("KustomizeTest: SmokeTest - Verify that gitops can deploy multiple apps one with private and other with public repo (e2e flow)", func() {
...
var _ = Describe("Weave GitOps Add Tests With Long Cluster Name", func() {
	...
	var _ = BeforeEach(func() {
		By("Given I have a brand new cluster with a long cluster name", func() {
			...
			kustomizeClusterName = "kind-123456789012345678901234567890"
			_, err = ResetOrCreateClusterWithName(WEGO_DEFAULT_NAMESPACE, deleteWegoRuntime, kustomizeClusterName)
			Expect(err).ShouldNot(HaveOccurred())
		})
	...
	It("KustomizeTest: SmokeTest - Verify that gitops can deploy an app with app-config-url set to <url>", func() {
		var repoAbsolutePath string
		var configRepoRemoteURL string
		var listOutput string
	...
	It("KustomizeTest: SmokeTest1 - Verify that gitops can deploy an app with app-config-url set to a gitlab <url>", func() {
		var repoAbsolutePath string
		var configRepoRemoteURL string
		var listOutput string
		var appStatus string
