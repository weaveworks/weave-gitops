...
var _ = XDescribe("WEGO Help Tests", func() {
	...
	It("Verify that gitops app add help flag prints the help text", func() {
		...
		By("Then I should see help message printed for gitops app add", func() {
			...
			Eventually(stringOutput).Should(MatchRegexp(`--auto-merge\s*If set, 'gitops add' will merge automatically into the set`))
			...
			Eventually(stringOutput).Should(MatchRegexp(`--dry-run\s*If set, 'gitops add' will not make any changes to the system; it will just display the actions that would have been taken`))
