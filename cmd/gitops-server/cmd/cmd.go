...
func runCmd(cmd *cobra.Command, args []string) error {
	...
	if server.AuthEnabled() {
		...
		if err != nil {
			return fmt.Errorf("failed to create HMAC signer: %v", err)
		}
		...
		if err := auth.RegisterAuthServer(mux, "/oauth2", srv, 1); err != nil {
			return fmt.Errorf("registering auth server: %w", err)
		}
	...
	mux.Handle("/", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if extension == "" {
			if server.AuthEnabled() {
				auth.WithAPIAuth(redirector, authServer, server.PublicRoutes).ServeHTTP(w, req)
			} else {
				redirector(w, req)
			}
			return
