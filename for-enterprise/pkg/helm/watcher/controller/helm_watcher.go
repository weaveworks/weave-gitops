...
func (r *HelmWatcherReconciler) checkForNewVersion(ctx context.Context, chart *pb.Profile) (string, error) {
	...
	newVersions, err := profiles.ConvertStringListToSemanticVersionList(chart.AvailableVersions)
	if err != nil {
		return "", err
	}

	oldVersions, err := profiles.ConvertStringListToSemanticVersionList(versions)
	if err != nil {
		return "", err
	}

	profiles.SortVersions(newVersions)
	profiles.SortVersions(oldVersions)
