...
#### GitOps Templates
- Support to specify Helm charts inside the CRD, instead of annotations.
...
#### Multi-Flux
- We can now support users running multiple instances of Flux on a single cluster for resource isolation, note the two versions within different namespaces on the cluster flux-system/dev

#### Minor fixes

- **Clicking on objects resulting in crashed UI**\
We highly recommend users upgrade to v0.12.0 from 0.11.0, as unfortunately a bug slipped in where clicking on objects in the details tab for reconciled resources often resulted in the UI displaying a raw error message.
