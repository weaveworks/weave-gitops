...
is based on the Kustomization controller of Flux.
...
to provision another resource to configure ACL for that bucket.

## Create a Terraform object

Similar to the same feature in the Kustomization controller, the dependency management feature is enabled 
by setting the `dependsOn` field in the `Terraform` object. The `dependsOn` field is a list of 
`Terraform` objects.
