...
WGE inspects the namespace in the management cluster where it is deployed, and looks for a `HelmRepository` objects. This Kubernetes objects should point to Helm chart repositories that include the Profiles available for installation.
