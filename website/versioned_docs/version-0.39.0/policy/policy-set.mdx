...
> Note: [Tenant Policies](./policy.mdx#tenant-policy) is always active in the [Admission](../weave-policy-profile#admission) mode, event if it is not selected in the `admission` policysets
