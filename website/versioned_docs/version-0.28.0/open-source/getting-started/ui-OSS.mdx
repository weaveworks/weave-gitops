...
- a `Kustomization` called `podinfo` corresponding to the application with the same name. The source referenced by this application is `shipping-service-podinfo` which has been verified.
- two `HelmReleases` called `weave-gitops-enterprise` and `tf-controller`, which deploys the respective Helm Charts.
