...
### Marking entire Flux HelmRepository resources as containing Profiles

Instead of marking individual Helm charts as Profiles, you can mark entire `HelmRepository` resources as providing Profiles.
...
```
Note that annotation values _must_ be strings so you can't use `true` here.
