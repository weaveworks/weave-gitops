...
> You would need to ensure that no other rolebindings or clusterrolebndings would allow read the the decryption secret at any time. This could be achieved by leveraging policy capabilities to detect existing and prevent future creation of roles that would grant read secrets permissions. 
