...
After you have choose your desired `SecretStore` & `SecretPath` the UI allows you to add secret properties in two different scenarios:

##### 1. Adding and Removing Multiple Property Fields

The first scenario allows you to add or remove multiple property fields. Each added `property` also has an optional `SecretKey` field. Here's how to do it:
...
Remember to use this option with caution.
