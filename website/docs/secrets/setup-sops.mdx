...
5- Configure the `decryption` part in the kustomization object with the private key created in step 3 and reference the public key created in step 4 in the annotations `sops-public-key/name` and `sops-public-key/namespace`. This is how WGE maps each public and private key pairs to manage the encryption/decryption flow.
