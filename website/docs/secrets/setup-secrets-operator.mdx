...
- To install the ESO profile the management cluster, `Add an application` from the `Applications` page and select `external-secrets` from `weaveworks-charts`.

#### Install ESO on leaf cluster

- To bootstrap the ESO profile on a leaf cluster, select `external-secrets` from the profiles dropdown in the Create Cluster page.
...
Bootstrapping SOPS to leaf cluster in WGE by utilizing ClusterBootstrapConfig job to bootstrap Flux and SOPS.
