...
Weave GitOps Enterprise now supports managing secrets using SOPS, a tool that encrypts and decrypts secrets using various key management services, from the [UI](/secrets/manage-secrets-ui.mdx). To be able to use this functionality, users need to configure their private and public key-pairs using one of the guides below.
...
- Visit the Secrets Page and start managing your secrets via the [UI](/secrets/manage-secrets-ui.mdx)
