...
#### GitOps Templates
- Support to specify Helm charts inside the CRD, instead of annotations. We’ve provided [documentation](.gitops.weave.works/docs/pipelines/pipeline-templates/) which has a example. 
...
- From the Pipelines view you can add new Pipelines in a way which leverages GitOpsTemplates, additionally - to help users configure these, we’ve provided [documentation](.gitops.weave.works/docs/pipelines/pipeline-templates/) which has some samples.

#### Support for multiple Flux instances on a single cluster
- Support for running multiple flux instances in different namespaces on a single cluster for better resource isolation.
...
If a customer chose to rename the local cluster as it is known to Weave GitOps Enterprise from the default of management there were certain workflows where errors would occur, such as fetching profiles, this has now been resolved.
