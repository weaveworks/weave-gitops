...
### 4.5 Configure access for writing to git from the UI
...
### 5.5 Configure password and add rolebindings for the

TODO: configure password

By default a user `wego-admin` is used when you login.

import ExampleAdminRolebinding from "./assets/example-enterprise-rolebindings.yaml";
import ExampleAdminRolebindingContent from "!!raw-loader!./assets/example-enterprise-rolebindings.yaml";

<CurlCodeBlock
  localPath="clusters/management/rbac/admin-rolebindings.yaml"
  hostedPath={ExampleAdminRolebinding}
  content={ExampleAdminRolebindingContent}
/>
