...
`GITHUB_TOKEN` or `GITLAB_TOKEN` should be set as an environment variable in the current shell. It should have permissions to create Pull Requests against the cluster config repo.
:::
...
CONFIGURE THE HELM repository and HELM chart
...
import ExampleAdminRolebinding from "!!raw-loader!./assets/example-enterprise-rolebindings.yaml";
import BrowserOnly from "@docusaurus/BrowserOnly";

<BrowserOnly>
  {() => (
    <CodeBlock className="language-bash">
      curl -o clusters/management/rbac/admin-rolebindings.yaml{" "}
      {window.location.protocol}
      //{window.location.host}
      {require("./assets/example-enterprise-rolebindings.yaml").default}
    </CodeBlock>
  )}
</BrowserOnly>

<CodeBlock
  title="clusters/management/rbac/admin-rolebindings.yaml"
  className="language-yaml"
>
  {ExampleAdminRolebinding}
</CodeBlock>
