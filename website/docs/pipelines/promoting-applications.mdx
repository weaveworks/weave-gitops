...
Creating pull requests requires read and write access in your git configuration repo.
Adopt [security recommendations](./#security) before using the feature.
:::
...
1. Pipeline controller receives events via webhook from leaf clusters. Hmac is used for authN/authZ so a hmac key should be provided in this case.
2. Pipeline controller clones and patches manifests to promote from the pipeline configuration repo via https or ssh. A set of [git credentials](https://fluxcd.io/flux/components/source/gitrepositories/#secret-reference) are required.
3. Pipeline controller uses git provider api to create the pull request with the promoted manifests. A Personal Access Token (PAT) to create to interact with pipelines git provider API.

The previous secrets are kept in a kubernetes Secret that we would call it promotion credentials.
...
Ensure you adopt the recommendations in this section to reduce the risks associated with the secrets involved in pull requests.
:::

1. **Do not use user accounts**: use service or bot accounts. User context from the credentials would be used to do any git provider operation and for
security and auditing perspective you don't want your user context be used in machine operations.
...
For example, if using GitHub, use [fine-grained tokens](https://github.blog/2022-10-18-introducing-fine-grained-personal-access-tokens-for-github/). Using classic tokens
would allow the token access to any repository that the user has permission too, increasing the exposed surface.

5. **Review active access tokens on a regular basis**: to ensure that only the ones that are required are present at all times.

6. **Reduce number of secrets to maintain**: Whenever ends in a more secure setup, favor git over https with tokens instead of ssh and deploy keys. Given that a token
is required for git provider api operations, to use git over ssh would require to secure and manage the deploy keys, as well as the token.
In addition, tokens have an expiration date (so requires rotation) and could have finer access control (to just allow the pipeline repo).

7. **Review git provider recommendations and examples**

See the following examples to help you to create minimal access credentials.

<details><summary>Expand to see examples</summary>

<Tabs groupId="git-provider" default>
<TabItem value="github" label="GitHub">
Github allows to create fine-grained access tokens that improves the security context provided by classic access tokens.
It allows to restrict to a single repository the access the token has access to. It also allows more granular
scopes.

In order to create a token for your application,

1. Use your service account or bot account.
2. Follow [create fine grained token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token#creating-a-fine-grained-personal-access-token)
2. Set Repository Access to only your configuration repo.
3. Set Permissions to read and write for [Contents](https://docs.github.com/en/rest/overview/permissions-required-for-fine-grained-personal-access-tokens?apiVersion=2022-11-28#contents)
    and [Pull Requests](https://docs.github.com/en/rest/overview/permissions-required-for-fine-grained-personal-access-tokens?apiVersion=2022-11-28#pull-requests)
