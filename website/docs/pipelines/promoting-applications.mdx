...
6. **Reduce number of secrets to maintain**: if you are using both ssh keys for git and tokens for git api, think
on using just tokens for both.  It would simplify the solution and reduce your surface of attack. Follow
this suggestion only when the overall security solution is a secure as the original one.
