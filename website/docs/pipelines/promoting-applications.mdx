...
Creating pull requests requires write access to your git repo. In case your pull request secret is compromised (and you could assume
it as a likely scenario), it could end up, for example, in deleting your production applications.

Please make sure you understand [Security](#security) section below before enabling automated pull requests.
:::
...
2. You have configured "Least Privilege" RBAC following [Kubernetes RBAC Good Practices](https://kubernetes.io/docs/concepts/security/rbac-good-practices/) with emphasis on the following:
...
by using Weave GitOps' [Policy capabilities](../../policy/intro/). Policy Agent acts in two complementary modes:
...
Once you have enabled Policy, the [Policy Library](../../policy/weave-policy-profile/) gives you have a set of policies to
help you with the task. `RBACProhibitWildcards` and `RBACProhibitVerbsOnResources` are building blocks
that you could use to implement RBAC least privilege.

Create using `RBACProhibitWildcards` the following policies by providing values to the parameters:

```yaml
apiVersion: pac.weave.works/v2beta2
kind: Policy
metadata:
  name: no-wildcard-on-resources-policy
spec:
  id: no-wildcard-on-resources-policy
...
kind: Policy
metadata:
  name: no-wildcard-on-verbs-policy
spec:
  id: no-wildcard-on-verbs-policy
  # omitted for simplicity
  parameters:
    - name: attributes
      type: string
...
Create using `RBACProhibitVerbsOnResources` the following policies by providing values to the parameters:

```yaml
apiVersion: pac.weave.works/v2beta2
kind: Policy
metadata:
  name: no-list-secrets-policy
spec:
  id: no-list-secrets-policy
  # omitted for simplicity
  parameters:
    - name: resource
      type: string
...
kind: Policy
metadata:
  name: no-watch-secrets-policy
spec:
  id: no-watch-secrets-policy
  # omitted for simplicity
  parameters:
    - name: resource
      type: string
...
kind: Policy
metadata:
  name: no-wildcard-secrets-policy
spec:
  id: no-wildcard-secrets-policy
  # omitted for simplicity
  parameters:
    - name: resource
      type: string
...
Add them to your configuration repo bases `./clusters/bases/policies`:
...
:::tip
Add an instance of this policy when onboarding a new pipeline.
:::
...
...
  parameters:
    - name: custom_namespace
      type: string
      required: true
      value: podinfo # changeme to the pipeline namespace
```
