...
you can use to enforce RBAC least privilege.
...
spec:
  ...
  # spec copied from <Policy Library>/RBACProhibitWildcards/policy.yaml omitted
  parameters:
    - name: attributes
      type: string
      required: true
...
You should consider segregating the namespace with pipelines from namespaces with workloads, to block this escalation path.
The Policy Library includes `weave.policies.containers-should-not-run-in-namespace` policy for that purpose:
