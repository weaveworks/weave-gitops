...
good practices policies that will help you securing your pipeline secrets according to previous RBAC recommendations. Deploy
...
Policies have exclusion features to accommodate privileged workloads like Flux. Make sure that you review them and remind
not allowing users to create RBAC resources without compliance checks. Otherwise, they could create RBAC resources that could escape this runtime control.
:::

In addition to escalation via RBAC, you should protect the escalation path [through workloads](https://kubernetes.io/docs/concepts/security/rbac-good-practices/#workload-creation):
...
You could do that by creating pipeline namespaces to hold the CR and secret without permission to run workloads. You
...
:::tip
Add an instance of this policy when onboarding a new pipeline with the correct namespace. Think on using Weave Gitops
self-service capabilities [GitOps Templates](https://docs.gitops.weave.works/docs/gitops-templates/intro/) or [GitOpsSets](https://docs.gitops.weave.works/docs/gitopssets/intro/)
