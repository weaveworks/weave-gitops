...
1. Pipeline controller receives events via [webhook from leaf clusters](./#setup-notifications-from-leaf-clusters). Hmac is used for authN/authZ so a hmac key should be provided in this case.
...
Git repository's URL and optionally a specific branch (if the branch is not set, it defaults to `main`).
