...
:::caution
Make sure that you understand and adopt as much of the [secret recommendations](./#promotion-credentials-recommendations) before creating it.
:::
...
#### Promotion credentials recommendations

//TODO see other system that create pull request, its recommendations

1. Do not use user accounts but service or bot accounts. User context from the credentials would be used to do any git provider operation and for
security and auditing perspective you don't want your user context be used in machine operations.

//TODO: add example

2. The promotion credentials needs to reside in the same Namespace as the Pipeline resource on the management cluster. Restrict
via RBAC that only the service account for pipeline controller is able to read it.

```yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata:
    name: <my-pipeline-pull-request>
...
kind: RoleBinding
metadata:
    name: pipeline-controller-<my-pipeline-pull-request>
    namespace: <my-pipeline-namespace>
roleRef:
...
3. Treat your tokens like passwords and keep them secured. Ensure that they expire, and you rotate them to reduce
the risks associated in case of being compromised.

4. Stick to the least privilege principle avoiding having high privilege tokens. Restrict the token to your just your repo and to just the operations required.
For example, if using GitHub, [fine-grained tokens](https://github.blog/2022-10-18-introducing-fine-grained-personal-access-tokens-for-github/) over classic or coarse-grained.
//TODO: add example of the creation of the token with minimum permissions

5. Review all currently active access tokens of all types on a regular basis and revoke any that are no longer needed.

6. Whenever ends in a more secure setup, favor git over https with tokens instead of ssh and deploy keys. Given that a token
...
7. Last, ensure that you stick to the best practices of your git provider
