...
We have only two tasks to demonstrate how to use parameter values from the
...
In out tasks, we are using only the `involvedObject.name`,
...
The template has the same parameters as the `Pipeline` resources has:
