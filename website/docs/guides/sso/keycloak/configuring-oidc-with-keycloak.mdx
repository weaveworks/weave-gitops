...
- A Kubernetes cluster runnnig a [Flux-supported version of Kubernetes](https://fluxcd.io/docs/installation/#prerequisites)
- Weave GitOps is [installed](https://docs.gitops.weave.works/docs/installation/) and [TLS has been enabled](https://docs.gitops.weave.works/docs/configuration/tls/)
...
![Creating a new Realm in Keycloak step 1](img/step_01.png)
...
![Creating a new Realm in Keycloak step 2](img/step_02.png)
...
![Keycloak realm homepage](img/step_03.png)
...
![Keycloak realm clients list](img/step_04.png)

Click on the `Create client` button at the top of the clients list.

![Keycloak create client step 1](img/step_05.png)

In the `General Settings` pane:

1. Set the `clientID` for your client to `weave-gitops`
2. Give your client a suggestive name
3. Click on the `Next` button.

![Keycloak create client step 2](img/step_06.png)
...
![Keycloak create client step 3](img/step_07.png)

Finally, in the `Login settings` pane:

1. Set the `Home URL` for your client to the URL of your Weave GitOps instance. For this demo, that's `https://gitops.mydomain.com`
2. Set the `Valid redirect URIs` to the URL of your Weave GitOps instance followed by `/oauth2/callback`. For this demo, that's `https://gitops.mydomain.com/oauth2/callback`
3. Click on `Save`
...
![Keycloak new client page](img/step_08.png)

From the `Clients` page, click on your newly created client.

![Keycloak client scopes](img/step_09.png)
...
![Keycloak create new mapper step 1](img/step_10.png)

In the new window, you should see that there are no mappers configured. Click on the `Configure a new mapper` button.

![Keycloak create new mapper step 2](img/step_11.png)

In the dialog that pops up, scroll down until you se `User Client Role` and select it.

![Keycloak create new mapper step 3](img/step_12.png)
...
![Keycloak new mapper in list](img/step_13.png)
...
![Keycloak create client role step 1](img/step_14.png)
...
![Keycloak create client role step 2](img/step_15.png)
...
![Keycloak create client role step 3](img/step_16.png)
...
![Keycloak new mapper in list](img/step_17.png)
...
![Keycloak create user step 1](img/step_18.png)
...
![Keycloak create user step 2](img/step_19.png)

In the new page that opens up, fill in the details of a demo user and click on `Create`.

![Keycloak create user step 3](img/step_20.png)
...
![Keycloak create user step 4](img/step_21.png)
...
![Keycloak create user step 5](img/step_22.png)

You should now see a password configured for your user.

![Keycloak create user step 6](img/step_23.png)
...
![Keycloak create user step 7](img/step_24.png)
...
![Keycloak create user step 8](img/step_25.png)
...
Once Weave GitOps is configured for OIDC, we need a way to map permissions to the groups. To do that, we need to create RBAC for our `wego-admin` group:
...
kind: RoleBinding
metadata:
  name: keycload-admin-group-read-resources
...
metadata:
  name: keycload-admin-group-read-resources-cr
subjects:
  - kind: Group
    name: wego-admin
    apiGroup: rbac.authorization.k8s.io
...
![Test config step 1](img/step_26.png)
