...
promoted or the update abandonned.  
...
![blue green mirroring icon](/img/mirroring.svg)  **Blue/Green with Traffic Mirroring** - sends
  copies of incoming requests to the new version. The user recieves the response from the current
...
- Flagger requires the `autoscaling/v2` API to be installed on the cluster, you can use `kubectl
...
:::note Make sure that any new directories that you add to the cluster repository as part of this
guide, are included in a path that Flux reconciles. :::
...
scale up a new version of the application (the canary), alongisde the existing version (the
...
the Canary defintion and created by Flagger:
