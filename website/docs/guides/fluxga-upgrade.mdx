...
### Can I Use Weave GitOps with Flux Beta?

From Weave Gitops v0.29 only Flux v2.0 GA is supported. Please follow the [Upgrade](#upgrade) section to help you with the process.

Earlier Weave Gitops versions works with both Flux GA and Flux Beta but is encouraged that you upgrade to latest for
the best experience.
