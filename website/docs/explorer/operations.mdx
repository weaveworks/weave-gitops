...
- `action` is the index write operation that could be either `Add`, `Remove`, `RemoveByQuery`
- `status` is the result of the operation. It could be either write operation that could be either `success` or `error`

```
indexer_latency_seconds_bucket{action="Add", le="+Inf", status="success"} 1
indexer_latency_seconds_bucket{action="Remove", le="+Inf", status="success"} 47
```
```
indexer_latency_seconds_sum{action="Add", status="success"} 0.008928666
indexer_latency_seconds_sum{action="Remove", status="success"} 0.06231312599999999
```
```
indexer_latency_seconds_count{action="Add", status="success"} 1
indexer_latency_seconds_count{action="Remove", status="success"} 47
```
