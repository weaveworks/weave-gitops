...
- `action` is the index read operation that could be either `GetObjects`, `GetAccessRules`, `GetObjectByID`, `GetRoles` or `GetRoleBindings`
- `status` is the result of the operation. It could be either  read operation that couild be either `success` or `error`
...
- `action` is the index read operation that couild be either `ListFacets` or `Search`
