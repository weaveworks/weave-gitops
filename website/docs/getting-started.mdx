...
-  an existing cluster bootstraped Flux 🎉
...
4. Now create a Kubernetes secret to store your chosen username and the password hash:

```
kubectl create secret generic admin-password-hash \
  --namespace flux-system \
  --from-literal=username=admin
  --from-literal=password='<output-from-step-3>'
```
...
4. Create a local values file for the Weave GitOps Helm Chart

```
cat > weave-gitops-values.yaml <<EOL
additional args:
- --insecure
rbac:
  create: true
...
5. Create a `HelmRelease` to deploy Weave GitOps

```
flux create helmrelease ww-gitops \
  --source=HelmRepository/ww-gitops \
  --chart=weave-gitops \
  --values=weave-gitops-values-values.yaml \
  --export > ./clusters/my-cluster/weave-gitops-helmrelease.yaml
```

6. Commit and push the `weave-gitops-helmrelease.yaml` to the `fleet-infra` repository
...
7. Validate that Weave GitOps and Flux are installed
...
2. [Open the dashboad](http://localhost:9001/) and login using either the cluster user or OIDC based on your [configuration](./configuration/securing-access-to-the-dashboard.mdx).
