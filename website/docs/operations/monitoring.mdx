...
Given Weave GitOps is writen in Go,profiling happens using [pprof](https://pkg.go.dev/runtime/pprof) that Weave GitOps
exposes via [pprof http handler](https://pkg.go.dev/net/http/pprof)
