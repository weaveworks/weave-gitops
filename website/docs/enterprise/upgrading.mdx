...
The command creates an IAM User `bootstrapper.cluster-api-provider-aws.sigs.k8s.io`. This user will be used as a kind of service account. It is assigned to the newly created group. The group name will be something like: `cluster-api-provider-aws-s-AWSIAMGroupBootstrapper-XXXX`. Then, create an AWS Access Key and Access Secret for the newly created IAM user.
...
Create a new Github repo (optional)
...
Export the bootstrap IAM user credentials created eariler
```
...
```
NOTE: The EKS may not allow the bootstrap user to access the cluster. Follow the [Enabling IAM user access to your cluster](https://docs.aws.amazon.com/eks/latest/userguide/add-user-role.html) guide 
to grant the bootstrap user access.
...
```bash
gitops upgrade --config-repo https://github.com/my-org/my-management-cluster --version 0.0.18
```
