...
To create new Pipelines and their required resources from within Weave GitOps Enterprise, you can leverage [GitOpsTemplates](../../gitops-templates/templates.mdx) to help platform teams scale for developer self-service. 
...
GitOpsTemplates are custom resources installed onto the management cluster where Weave GitOps Enterprise resides. To add a new Pipeline, click `Add Pipeline` from within the Pipeline view, which will take you to a pre-filtered list of templates with the label: `weave.works/template-type: pipeline`.
...
This example builds on the above to add a promotion strategy, in this case it will raise a pull request to update the application version in subsequent environments.
...
For the `Provider`, this template allows the user to choose between unauthenticated (Generic) and authenticated (HMAC) communication to the promotion endpoint. In the authenticated use case, a secret will need to be present on both the management cluster and leaf cluster(s).
...
For full details on GitOpsTemplates, be sure to read our [documentation](../../gitops-templates/templates.mdx).
