...
Weave GitOps Enterprise works with standard Kubernetes RBAC for authorization.
...
<TabItem value="OIDC" label="Manual OIDC">
...
<TabItem value="Cluster User" label="Manual Cluster User">
...
output as part of the bootstrapping session:
...
Other networking configurations could be achieved by modifying the `service` or `ingress` in the `HelmRelease` values:
...
### Install Weave GitoOps Enterprise