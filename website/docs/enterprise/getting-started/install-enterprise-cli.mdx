...
6. [Configure Dashboard Access](#select-the-domain): choose between 2 methods to access the dashboard either local or external.
...
1. **Service**: this option is called `localhost` in the cli and the dashboard will be available through [ClusterIP Kubernetes Service](https://kubernetes.io/docs/concepts/services-networking/service/#type-clusterip).
2. **Ingress**: this option is called `externaldns` the dashboard will be available through [Ingress](https://kubernetes.io/docs/concepts/services-networking/ingress/) with the following considerations:
    - An Ingress controller needs to exist.
    - A host-based ingress will be created of the ingress class `public-nginx`.
    - An externaldns annotation will be added with the value of the provided domain.
