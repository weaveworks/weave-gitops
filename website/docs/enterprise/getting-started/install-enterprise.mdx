...
- **Git Repository**: the Git configuration repo to be used by Flux and Weave GitOps.
- **Management Cluster**: a Kubernetes cluster with a Kubeconfig with cluster admin permissions to be able to create resources.
- **CLI Tools**: install Flux and Weave GitOps CLI.
...
Follow https://fluxcd.io/flux/installation/#install-the-flux-cli

### Install Weave GitOps CLI

To do this, you can use either brew or curl:

    <Tabs groupId="weave-gitops-cli">
...
At glance getting started with Weave GitOps Enterprise requires the following steps:

1. [Git Repository](#configure-git-access)
2. [Bootstrap Flux](#bootstrap-flux)
3. [Entitlements](#verifying-entitlement)
4. [Authentication & Authorization](#create-cluster-user)
5. [Install WGE](#configure-dashboard-access)
6. [Access the dashboard](#access-the-dashboard)

You could transit the process via the following routes:

- `CLI Interactive`: you want to be guided step-by-step into configuring Weave GitOps Enterprise. Suitable for when you have limited knowledge about Weave GitOps Enterprise.
- `CLI Non-Interactive`: you want to leverage cli for automation with interactive. Suitable for when you have already some knowledge about Weave GitOps Enterprise.
- `Manual`: you want to have full control on the bootstrapping process. Suitable when the CLI cannot be used or wanted to oversee any step.
    ...
    <TabItem value="manual" label="manual">
        For manual installation or deep dive on any of the stages, you could follow the below sections:
    </TabItem>
    </Tabs>
...
### Git Repository
<Tabs groupId="git repository" default>
    <TabItem value="cli" label="cli">

        ```
        # bootstrap WGE using repo
        gitops bootstrap --repo-url="https://example.com/repository.git" --branch="main" --repo-path="clusters/management""
...
The `flux bootstrap` command enables you to deploy Flux on a cluster the GitOps way:
See other options [Flux bootstrap](https://fluxcd.io/docs/cmd/flux_bootstrap/) documentation.
...
To get the entitlement secret please contact *sales@weave.works*, then apply it on your management cluster with the name `weave-gitops-enterprise-credentials` under `flux-system` namespace.
   ...
   <Tabs groupId="entitlments" default>
   <TabItem value="manual" label="manual">
   ...
   <TabItem value="cli" label="cli">
    It checks that the secret exists on the management cluster, and that it is valid will check if it has valid content and the entitlement is not expired.
   </TabItem>
   </Tabs>
...
- `Cluster User`: which is insecure, and which we only recommend for local and development environments or if you need to activate emergency access to a damaged cluster. However, it is an option if an OIDC provider is not available.

The both work with standard Kubernetes RBAC for authorization.

<Tabs groupId="authN/authZ" default>
<TabItem value="CLI" label="CLI">
...
## Advanced Configurations or Day 1

The following stages applies

1. [Extra Components](#extra-components): to install extra components from (Policy Agent, Terraform Controller).
