...
- install Weave GitOps Enterprise, using the `gitops-ee bootstrap` command
...
cluster user](../../configuration/emergency-user.mdx) credentials or OIDC. Then it makes Kube API calls on the user's
...
and qa-team. This also implicitly disables the [emergency cluster user](../../configuration/emergency-user.mdx)).
...
We don't recommend exposing services without TLS. Without a certificate, you
can't be sure if you're using the correct service, and the traffic will be easily
