...
## Example: Set up a Management Cluster with `flux`
...
To get you started, we'll cover EKS as our management cluster with the _CAPA_ provider. Please note again that Weave GitOps Enterprise supports [clusters without Cluster API](../../cluster-management/managing-existing-clusters/), as well as any combination of management cluster and CAPI provider.

### 1. Prepare IAM for Installation
...
### 3. Add the Cluster to kubeconfig
...
Generally speaking, you do not need to install a CAPI provider to provision Kubernetes clusters using Weave GitOps Enterprise. But for this example with CAPA, you must. Visit the [Cluster API Providers](../../cluster-management/cluster-api-providers.mdx) page for more details on providers.
