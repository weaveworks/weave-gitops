...
To get you started, we'll cover **EKS** as our management cluster with the _CAPA_ provider. Please not again that Weave GitOps Enterprise supports any combination of management cluster and CAPI provider, as well as [clusters without Cluster API](../../cluster-management/managing-existing-clusters/).
