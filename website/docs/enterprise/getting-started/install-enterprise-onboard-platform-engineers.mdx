...
:::info What to expect
You have a baseline WGE environment that you want to iterate towards production-ready, so you could onboard
other Platform Engineers and finally Developers.

Use this guide to walk the bootstrapping workflow and adopt the recommendations that satisfy your context.
:::
...
- `Login via Cluster User`: that you have likely setup already as part of your previous step.
- `Login via OIDC`: it is the recommended way for production, as this will allow you to control permissions for existing users and groups that have
...
a central system designed for that purpose (i.e. the OIDC provider).

Making WGE authentication fit for production means setting up OIDC.
...
As a result of getting started you have create the default authorization clusters for `Cluser User`, `Weave GitOps Service Account`
and `Flux Service Accounts`.

Making WGE authorization fit for production involves [defining and Deploy RBAC for OIDC users](./#rbac-for-oidc-users

### RBAC for OIDC Users

#### Define RBAC Rules

RBAC rules are highly context dependent so you would need to take the time with your Security organisation defining them.

You could use this example an initial inspiration in case you have nothing already defined

#### Deploy RBAC Rules

A common pattern called `Bases` helps you deploying your OIDC RBAC:
