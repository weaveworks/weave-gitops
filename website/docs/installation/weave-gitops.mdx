...
## How to Install the Open Source Version of Weave GitOps on Your Cluster

:::tip
These instructions only apply to the open source tier of Weave GitOps. To install Weave GitOps Enterprise, [go here][ee-install].
:::

### Prerequisites

Before you can install open source (OSS) Weave GitOps, you will need:
...
then see their docs on [setting your username](https://docs.github.com/en/get-started/getting-started-with-git/setting-your-username-in-git#setting-your-git-username-for-every-repository-on-your-computer) and 
...
1. Install Flux onto your cluster with the `flux bootstrap` command. `flux bootstrap` creates a `flux system` folder in your repository that includes the manifests Flux needs to operate. It also generates a key value pair for Flux to access the repo. 

The command below assumes the Git provider to be `github`. Change this if you would rather use
	`gitlab`.
...
1. If you have difficulty saving the YAML to the correct path, run the command `mkdir -p ./clusters/my-cluster`.
...
1. Run the following command, which will create a `HelmRepository` and `HelmRelease` to deploy Weave GitOps
   ...
   ```
	...
	:::warning
	This command stores a hash of a password.  This is relatively safe for demo
	and testing purposes, but we strongly recommend using a more secure method of storing secrets
...
You will use the password you've just created when you've finished Weave GitOps OSS installation and are ready to login to the dashboard UI.
