...
For more information about Weave GitOps Enterprise Edition, see the [Enterprise
feature page](@site/enterprise/intro-ee.mdx).
:::
...
In order to be able to provision Kubernetes clusters, a CAPI provider needs to
be installed. See [Cluster API Providers](@site/enterprise/cluster-management/cluster-api-providers.mdx) page for more details on providers.
...
import ExampleWGE from "@site/docs/assets/example-enterprise-helm.yaml";
import ExampleWGEContent from "!!raw-loader!@site/docs/assets/example-enterprise-helm.yaml";
...
[Policy agent](@site/enterprise/policy/intro.mdx) comes packaged with the WGE chart. To install it you need to set the following values:
...
In our following [Get Started document](@site/docs/getting-started/ui.mdx), we will walk you through logging into the GitOps Dashboard and deploying an application.

Then you can head over to either:

- [Cluster Management - Getting started](@site/enterprise/cluster-management/getting-started.mdx) to create your first CAPI Cluster with `kind`/CAPD
- [Deploying CAPA with EKS](@site/enterprise/guides/deploying-capa.mdx) to create your first CAPI Cluster with EKS/CAPA.
...
Check out our guide on how to [use Terraform templates](@site/enterprise/guides/using-terraform-templates.mdx), and why not try your hands at using it with the RDS example!
