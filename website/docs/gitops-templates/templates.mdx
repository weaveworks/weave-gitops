...
Templates can be rendered to multiple paths in the repository. This allows for more control over where different resources in the template are rendered.

### Specifying paths for `resourcetemplates`
...
- **templating**
  ...
  | ------------------------------- | ------------------------------------------------------------------------------------------ |
  | String Functions                | _trim_, _wrap_, _randAlpha_, _plural_                                                      |
  | String List Functions           | _splitList_, _sortAlpha_                                                                   |
  | Integer Math Functions          | _add_, _max_, _mul_                                                                        |
  | Integer Slice Functions         | _until_, untilStep                                                                         |
  | Float Math Functions            | _addf_, _maxf_, _mulf_                                                                     |
  | Date Functions                  | _now_, _date_                                                                              |
  | Defaults Functions              | _default_, _empty_, _coalesce_, _fromJson_, _toJson_, _toPrettyJson_, _toRawJson_, ternary |
  | Encoding Functions              | _b64enc_, _b64dec_                                                                         |
  | Lists and List Functions        | _list_, _first_, _uniq_                                                                    |
  | Dictionaries and Dict Functions | _get_, _set_, _dict_, _hasKey_, _pluck_, _dig_, _deepCopy_                                 |
  | Type Conversion Functions       | _atoi_, _int64_, _toString_                                                                |
  | Flow Control Functions          | _fail_                                                                                     |
  | UUID Functions                  | _uuidv4_                                                                                   |
  | Version Comparison Functions    | _semver_, semverCompare                                                                    |
  | Reflection                      | _typeOf_, _kindIs_, _typeIsLike_                                                           |
