...
Templates are simple YAML files, that can be enriched with Parameters, Variables, Metadata and conditions. They can be rendered to create the resources they contain. For clusters it can be CAPI objects like MaschinePool. It can be as well Kustomization (flux) or a TF controller resource. 
...
**Why do I care about GitOps templates?**

Originally introduced enabling helping us in the cluster creation flow, enabling platform operators to create clusters that have guardrails and are repeatable we started to leverage templates as well for Terraform controller. 

We learned that we can literally template anything. Imagine a template that provides a running Developer Environment, consisting in a EKS cluster, a RDS Database, and a branch + revision of the current application through a single template. 
