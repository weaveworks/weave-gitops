...
# GitOpsTemplates <TierLabel tiers="enterprise" />

`GitOpsTemplates` API defines a template that can be used to create any Kubernetes resources.  



The `GitOpsTemplates` API has the ability to define a list of resource that will be templated through the `spec.resourceTemplates` field.
```
...
The `GitOpsTemplates` API also has the ability to set default and selectable parameters.
