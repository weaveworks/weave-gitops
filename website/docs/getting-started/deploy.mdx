...
:::info
Podinfo comes with the HorizontalPodAutoscaler, which uses the `metrics-server`.
We don't use the `metrics-server` in this tutorial, but not that it's the reason why HorizontalPodAutoscaler will report as `Not ready` in your dashboard. We recommend ignoring the warning.
:::
