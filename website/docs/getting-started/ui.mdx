...
1. [Open the dashboard](http://localhost:9001/) and login using either the [emergency cluster user](../configuration/emergency-user.mdx)
	or the OIDC, based on your [configuration](../configuration/securing-access-to-the-dashboard.mdx). 
	If you followed the example above, the emergency user will be configured with the username set to `admin`. This means that you can use “admin” as your user name, and the password that you set earlier during installation.
...
Now that we are familiar with the WGE dashboard, let's deploy a new application :sparkles:.
