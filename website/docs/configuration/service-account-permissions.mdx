...
- apiGroups: [""]
  ...
  resourceNames:                  # set by rbac.viewSecretsResourceNames
    - "weave-gitops-user-auth" # the name of this Secret depends on your configuration, see below for details.
    - "oidc-auth"
...
* read the `weave-gitops-user-auth` and `oidc-auth` secrets, which are the secrets
  storing the cluster-user account and OIDC configuration (see
...
## Reading the admin user and oidc-auth secrets

The admin user and oidc-auth secrets provide information for authenticating
to the application. The former holds the username and bcrypt-hashed password
for the static user and the later holds OIDC configuration. The admin user secret name
is configurable and its name depends on your specific setup.
