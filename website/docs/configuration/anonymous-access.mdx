...
If you decide to use anonymous access, you should be cautious about exposing the endpoint and consider implementing additional authentication measures such as an auth proxy.
:::
...
The value of the `--insecure-no-authentication-user` flag is the
kubernetes `User` that will be impersonated to make requests into the cluster.
