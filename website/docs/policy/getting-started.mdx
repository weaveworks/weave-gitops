...
Enabling the Weave Policy Engine features in Weave GitOps is done by running the policy agent on the cluster. This section details the steps required for installing and running the policy agent on the management and leaf clusters.
...
## Install the policy agent on the management cluster

The agent comes packaged with the WGE chart. To install it you need to set `values.policy-agent.enabled` to `true`.

## Install the policy agent on a leaf cluster
