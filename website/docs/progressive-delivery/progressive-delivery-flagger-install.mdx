...
The Applications > Delivery view of the Weave GitOps Enterprise UI enables you to view the status of
progressive delivery rollouts and how they're configured, leveraging Flagger's
[canary](https://docs.flagger.app/usage/how-it-works#canary-resource) object. View near-to-realtime data on any summary or details page.
...
- click on the yaml tab on the detail page to see the raw yaml of  canary
- enable the progressive delivery feature through a feature flag
...
To install Linkerd we'll use a Kustomization file. It will allow us to specify the order and default namespace for the installed resources, and to easily generate Secrets from certificate files via the use of a `secretGenerator`.
